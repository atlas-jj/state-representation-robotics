...
task_names = ['BlocksV2', 'Cloth_prev'] #['Toy_Example', 'BlocksV2', 'Cloth_prev'] #['Blocks', 'Plug', 'Toy_Example]
...
data_sets = [data_set2, data_set3]  #[data_set1, data_set2]
sample_sizes = [sample_size2, sample_size3]  #[sample_size1, sample_size2]
sample_sequence_folders = ['human1', 'human2']
sample_sequence_numbers = [205, 111]
dim_Zs = [50, 100]
...
def generate_file_name(task_name, data_set, dim_Zt, epoch, bce):
    return "VAE_" + task_name+'_'+data_set+'_dim'+str(dim_Zt)+'_' + str(epoch)+'_'\
           + uts.d2s(bce, 2)
...
def train(epoch, train_loader, model, optimizer, task_name, data_set, dim_Zt, plot_r):
    ...
    for batch_idx, (data) in enumerate(train_loader):
        ...
        loss, bce, kld = models.loss_function(recon_batch, data, mu, logvar)  # from loss function to back prop the variables.
        loss.backward()
        this_loss = loss.item()
...
def test(epoch, test_loader, model, task_name, data_set, dim_Zt, plot_r):
    ...
    with torch.no_grad():
        for i, (data) in enumerate(test_loader):
            ...
            loss, bce, kld = models.loss_function(recon_batch, data, mu, logvar)
            test_loss += loss.item()
    ...
    for i in range(len(task_names)): # task_level
        ...
        for j in range(len(data_set)): #data_set level
            ...
            for m in range(len(dim_Zs)):  # dim Z level
                ...
                model = models.VAE(dim_Zt, args.input_channel).to(device)
                # initialize an optimizer
                optimizer = optim.Adam(model.parameters(), lr=5e-4)
                plot_r = pr.plot_task_map(dim_Zt, '../../Experiments/Dataset/' + task_name, sample_sequence, device)
                train_losss = []
                ...
                # save the model
                model.save("VAE_" + task_name + "_" + data_set_name + "_dimZ_" + str(dim_Zt))
                np.save("train_loss_VAE_" + task_name + "_" + data_set_name + "_dimZ_" + str(dim_Zt), train_losss)
