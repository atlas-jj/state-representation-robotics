...
def generate_file_name(task_name, data_set, dim_Zt, epoch, bce):
    return "SAE_" + task_name+'_'+data_set+'_dim'+str(dim_Zt)+'_' + str(epoch)+'_'\
           + uts.d2s(bce, 2)
...
def train(epoch, train_loader, model, optimizer, task_name, data_set, dim_Zt, plot_r):
    ...
    print('Train:{} {} dim {} Epoch:{} [{}/{} ({:.0f}%)]\tLoss: {:.6f}\t[BCE: {:.6f}]'.format(
        task_name, data_set, dim_Zt, epoch, (batch_idx + 1) * len(data), len(train_loader.dataset),
                                            100. * (batch_idx + 1) / len(train_loader), train_loss, train_loss))
    if epoch % epoch_interval == 0:
        plot_r.save_results_4(model, generate_file_name(task_name, data_set, dim_Zt, epoch, train_loss),
    ...
    for i in range(len(task_names)): # task_level
        ...
        for j in range(len(data_set)): #data_set level
            ...
            for m in range(len(dim_Zs)):  # dim Z level
                ...
                model = models.SpatialAutoEncoder(dim_Zt, args.input_channel).to(device)
                ...
                # save the model
                model.save("SAE_" + task_name + "_" + data_set_name + "_dimZ_" + str(dim_Zt))
                np.save("train_loss_SAE_" + task_name + "_" + data_set_name + "_dimZ_" + str(dim_Zt), train_losss)
